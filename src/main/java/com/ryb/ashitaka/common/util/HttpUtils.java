...
public final class HttpUtils {
     ...
     * 定义请求客户端
     */
    private static final OkHttpClient client = new OkHttpClient().newBuilder().addInterceptor(new HttpLoggingInterceptor().setLevel(HttpLoggingInterceptor.Level.BODY)).build();
    ...
    public static String doPost(String url, Map<String, Object> header, Map<String, Object> parameter, File file, String fileFormName) throws Exception {
        ...
        if (null != file) {
            ...
            requestBuilder.addFormDataPart(StrUtil.isNotBlank(fileFormName) ? fileFormName : "file", file.getName(), RequestBody.create(MediaType.parse("application/octet-stream"), file));
        }
    ...
    public static String doPost(String url, Map<String, Object> header, Map<String, Object> parameter, String fileName, byte [] fileByte, String fileFormName) throws Exception {
        ...
        if (fileByte.length > 0) {
            // application/octet-stream
            requestBuilder.addFormDataPart(StrUtil.isNotBlank(fileFormName) ? fileFormName : "file", fileName, RequestBody.create(MediaType.parse("application/octet-stream"), fileByte));
        }
